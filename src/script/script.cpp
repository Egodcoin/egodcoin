...
const char* GetOpName(opcodetype opcode)
{
    ...
    case OP_EGOD_ASSET             : return "OP_EGOD_ASSET";
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_EGOD_ASSET) { // OP_EGOD_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == EGOD_R) { // Check to see if EGOD starts at 27 ( this->size() < 105)
                if ((*this)[28] == EGOD_V)
                    if ((*this)[29] == EGOD_N)
                        index = 30;
            } else {
                if ((*this)[28] == EGOD_R) // Check to see if EGOD starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == EGOD_V)
                        if ((*this)[30] == EGOD_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == EGOD_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == EGOD_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == EGOD_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == EGOD_R) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_EGOD_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_EGOD_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_EGOD_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_EGOD_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_EGOD_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_EGOD_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
