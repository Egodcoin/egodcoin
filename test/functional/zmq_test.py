...
from test_framework.mininode import yerbashash
...
def yerbashash_helper(b):
    return encode(yerbashash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def _zmq_test(self):
        ...
        assert_equal(genhashes[0], yerbashash_helper(body[:80]))
        ...
        for x in range(n * 4):
            ...
            if topic == b"hashblock":
                ...
                blockcount += 1
            if topic == b"rawblock":
                zmqRawHashed.append(yerbashash_helper(body[:80]))
                msgSequence = struct.unpack('<I', msg[-1])[-1]
